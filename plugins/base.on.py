...
def _parse_positions_traj_time(
    path: str,
) -> Tuple[Dict[int, List[Tuple[str, float, float, float]]], List[str], int, List[int]]:
...
def run_import_positions(task, args):
    ...
    proc.append(
        f"[位置] 文件夹：{os.path.abspath(folder)}｜轨迹ID：{traj.traj_id}｜帧(位置)：{len(steps)}｜原子：{n_atoms}"
    )
    return {"process": proc}
...
def _parse_velocities_vel_time(
    path: str,
) -> Tuple[Dict[int, List[Tuple[float, float, float]]], int, List[int]]:
    ...
    while i < len(lines):
        ...
        for _ in range(n):
            ...
            parts = lines[i].split()
            if len(parts) >= 4:
                vx = _to_float(parts[1])
                vy = _to_float(parts[2])
                vz = _to_float(parts[3])
                lst.append((vx, vy, vz))
        if step is not None:
            vel_data[step] = lst
            steps.append(step)
...
def run_import_velocities(task, args):
    ...
    traj.refresh_basic_meta()
    proc.append(
        f"[速度] 文件夹：{os.path.abspath(folder)}｜轨迹ID：{traj.traj_id}｜帧(速度)：{len(steps)}｜原子：{n_atoms}"
    )
    return {"process": proc}
...
def _vec(r, i):
    x = _to_float(r.get(f"x_{i}"))
    y = _to_float(r.get(f"y_{i}"))
    z = _to_float(r.get(f"z_{i}"))
    return (x, y, z)
...
def run_geom(task, args):
    ...
    if not raw:
        ...
        out = args.get("out", "").strip()
        raw = " ".join([kind or "", group.replace(",", " "), out]).strip()
    parts = [p for p in re.split(r"[,\s]+", raw.strip()) if p]
    if not parts:
        raise ValueError("命令为空")
    kind = parts[0].lower()
    if kind not in ("dist", "angle", "dihedral"):
        raise ValueError("类型应为 dist/angle/dihedral")
    ...
    for tid, traj in task.trajectories.items():
        ...
        for r in traj.table.rows:
            if kind == "dist":
                ...
                vj = _vec(r, j)
                vk = _vec(r, k)
                if None in vi or None in vj or None in vk:
                    r[col] = None
                else:
                    ...
                    if ni == 0 or nk == 0:
                        ...
                        cs = (vji[0] * vjk[0] + vji[1] * vjk[1] + vji[2] * vjk[2]) / (
                            ni * nk
                        )
                ...
                rk = _vec(r, k)
                rl = _vec(r, l)
                if None in ri or None in rj or None in rk or None in rl:
                    r[col] = None
                else:
...
def run_com(task, args):
    ...
    for tid, traj in task.trajectories.items():
        ...
        cols = set(traj.table.columns)
        cols |= {"com_x", "com_y", "com_z"}
        for r in traj.table.rows:
            sx = sy = sz = 0.0
            msum = 0.0
            for i in range(1, n + 1):
                xi = _to_float(r.get(f"x_{i}"))
                yi = _to_float(r.get(f"y_{i}"))
                zi = _to_float(r.get(f"z_{i}"))
                if None in (xi, yi, zi):
                    continue
...
def run_stats(task, args):
    ...
    if raw:
        ...
        column = parts[0]
        skip_none = (
            parts[1].lower() not in ("false", "0", "no") if len(parts) >= 2 else True
        )
    else:
    ...
    for tid, traj in task.trajectories.items():
        if column not in traj.table.columns:
            traj.meta[list_field] = None
            traj.meta[f"var__{column}"] = None
            proc.append(f"[统计] 轨迹ID：{tid}｜列不存在：{column}")
            continue
        ...
        proc.append(
            f"[统计] 轨迹ID：{tid}｜列：{column}｜均值：{traj.meta[list_field]}｜方差：{traj.meta[f'var__{column}']}"
        )
    return {"process": proc}
...
def run_global(task, args):
    ...
    proc: List[str] = [
        f"[全局] 列：{column}｜全局均值：{task.meta[f'global_mean__{column}']}"
    ]
    if sync:
        for tid, traj in task.trajectories.items():
...
def safe_eval(expr, names, task):
    ...
    def chk(n):
        ...
        if isinstance(n, ast.Expression):
            ...
            chk(n.operand)
        elif isinstance(n, ast.BinOp) and isinstance(
            ...
            if not isinstance(n.func, ast.Name):
                raise ValueError("不支持函数类型")
            fn = n.func.id
            if fn not in SAFE and fn not in ("mean", "sum", "min", "max"):
                raise ValueError(f"函数不允许：{fn}")
            for a in n.args:
                chk(a)
            if n.keywords:
                raise ValueError("不支持关键字参数")
        elif isinstance(n, ast.Name):
            if n.id not in names and n.id not in ("mean", "sum", "min", "max"):
                raise ValueError(f"未知变量：{n.id}")
        elif isinstance(n, ast.Constant) and isinstance(n.value, (int, float)):
            return
        else:
            raise ValueError("不支持表达式")
        ...
        def visit_Call(self, call):
            ...
            if isinstance(call.func, ast.Name) and call.func.id in (
                ...
                if len(call.args) != 1 or not isinstance(
                    call.args[0], (ast.Str, ast.Constant)
                ):
...
def run_expr(task, args):
    ...
    if not name or not expr:
        raise ValueError("缺少 name 或 expr")
    names = dict(task.meta)
    names["traj_count"] = len(task.trajectories)
...
PLUGINS = [
    ...
    {
        "name": "导入-位置",
        "description": "读取 traj_time.out（单文件夹）",
        "scope": "Import",
        "run": run_import_positions,
        "input": {"mode": "line", "help": "无需输入，主程序传入 folder", "example": ""},
    },
    {
        "name": "导入-速度",
        "description": "读取 vel_time.out（单文件夹）",
        "scope": "Import",
        "run": run_import_velocities,
        "input": {"mode": "line", "help": "无需输入，主程序传入 folder", "example": ""},
    },
    # 计算
    {
        "name": "几何计算",
        "description": "距离/角度/二面角（10位小数）",
        "scope": "Trajectory-Frame",
        "run": run_geom,
        "input": {
            "mode": "line",
            "help": "dist i j [out]｜angle i j k [out]｜dihedral i j k l [out]",
            "example": "dist 1 2 d_12",
        },
    },
    {
        "name": "质心计算",
        "description": "COM（10位小数）",
        "scope": "Trajectory-Frame",
        "run": run_com,
        "input": {
            "mode": "line",
            "help": "masses m1,m2,...（留空=默认质量）",
            "example": "masses 12,1,1,16",
        },
    },
    {
        "name": "轨迹统计",
        "description": "均值/方差（10位小数）",
    ...
    {
        "name": "全局统计",
        "description": "任务级均值（10位小数，可同步到轨迹）",
        "scope": "Task-Global",
        "run": run_global,
        "input": {
            "mode": "line",
            "help": "<列名> [sync_to_trajs]，如：dist_1_2 true",
            "example": "dist_1_2",
        },
    },
    {
        "name": "任务表达",
        "description": "表达式（10位小数，安全评估）",
        "scope": "Task-Global",
        "run": run_expr,
        "input": {
            "mode": "line",
            "help": "name <结果名> expr <表达式> [sync_to_trajs true] 或 key=value",
            "example": 'name gm expr mean("global_mean__dist_1_2")',
        },
    },
]
